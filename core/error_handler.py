...
All exceptions are forwarded to the structured logging system (app_logging.py):
    - Exception details  → log/exceptions.log
    - UX action outcomes → log/ux_actions.log
"""
...
def safe_ui_action(
    ...
    Args:
        ...
        log_ux: If True, logs action start/result to UX actions log.
                Set to False for per-row computations that aren't real user actions.
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            # Log UX action start
            if log_ux:
                log_ux_action(action, details=f"invoked {func.__qualname__}")
            try:
                result = func(*args, **kwargs)
                # Log UX action success
                if log_ux:
                    log_ux_action_result(action, success=True)
                return result
                ...
                error_msg = str(exc)
                # Log to legacy app logger
                logger.error(
                    f"Error in {action}: {error_msg}",
                    exc_info=log_full_traceback,
                )
                # Log to structured exception log
                _log_exc_to_file(action, exc, context=f"decorator=safe_ui_action func={func.__qualname__}")
                # Log UX action failure (always log failures, even if log_ux=False)
                log_ux_action_result(action, success=False, details=error_msg)
                ...
                if show_error_dialog:
                    ...
                    try:
                        ...
                        _exc_logger.exception(f"Failed to show error dialog: {dialog_exc}")
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                # Log to structured exception log
                _log_exc_to_file(action, exc, context=f"decorator=safe_ui_action_returning func={func.__qualname__}")
                # Log UX action failure (always log failures, even if log_ux=False)
                log_ux_action_result(action, success=False, details=error_msg)
...
def wrap_action_with_error_handling(
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        action = action_name or func.__name__
        log_ux_action(action, details=f"callback {func.__qualname__}")
        try:
            result = func(*args, **kwargs)
            log_ux_action_result(action, success=True)
            return result
            ...
            _log_exc_to_file(action, exc, context=f"wrapper func={func.__qualname__}")
            log_ux_action_result(action, success=False, details=error_msg)
